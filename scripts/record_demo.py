...
from vigia.web.overlay import draw                            # noqa: E402
...
def compose(annotated, state: dict, spec: dict) -> np.ndarray:
    ...
    y = top + 52
    for index, name in enumerate(("confidence", "colour", "persistence", "cooldown"), 1):
...
def render(spec: dict, clips_dir: Path, out: Path, fps: float) -> dict:
