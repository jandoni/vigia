\section{Definitions and evaluation metrics}
\label{sec:metrics}
...
rather than of model: a figure that was correct as computed and wrong as
interpreted. Stating the definitions precisely is what made those visible.

\subsection{Matching predictions to ground truth}
...
\subsection{Two levels of evaluation, and why both}
\label{sec:twolevels}
...
\subsection{Recall-weighted \texorpdfstring{$F$}{F} scores}
...
\subsection{Segmentation, and the trap of pooling}
\label{sec:pooling}
...
makes concrete rather than academic~\citep{brown2001interval}. At
...
Computed by \texttt{docs/build\_paper.py}, not typed.}
\label{tab:intervals}
