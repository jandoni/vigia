...
experiment for the validator, not a headline capability — a car
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{qual_fire.png}
\caption{These qualitative panels are illustrative, not evidential: each is the frame with the most confirmed detections in its clip, and the paper's quantitative claims rest on the tables. A confirmed wildfire-smoke detection on a HPWREN FIgLib ignition
sequence, rendered by the same overlay the running system uses
(\texttt{vigia/web/overlay.py}). The plume at the right edge has persisted
across three frames and is confirmed; the corner brackets and confidence chip
are the operator-view encoding. Imagery: HPWREN, University of California San
Diego — attribution required.}
\label{fig:qual-fire}
\end{figure}
...
\subsubsection{Flood and water level}
\label{sec:flood}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{qual_flood.png}
\caption{Confirmed flood on a USGS streamgage camera during the August 2024
Walnut Creek event. The segmentation mask is shown with its boundary; the
connected water regions above the minimum-area threshold are boxed and
confirmed. Showing the mask, and not only the derived boxes, reflects that
the detector's native output is a mask (\cref{sec:pooling}). Imagery: USGS,
public domain.}
\label{fig:qual-flood}
\end{figure}
...
\begin{recorded}{Sliced inference: more recall, far more false alarms}
The fixed \SI{512}{\pixel} input means a swimmer at altitude reaches the
model at less than half native resolution, which is the textbook case for
sliced inference (SAHI~\citep{akyon2022sahi}): run the detector on
overlapping native-resolution tiles and merge. Measured on the same split
(\texttt{eval/run\_sahi\_eval.py}), it did raise swimmer recall from
\SahiBaseRecall{} to \SahiSlicedRecall. It also collapsed precision
(\SahiBasePrecision{} to \SahiSlicedPrecision), drove the image-level
false-alarm rate up by a factor of \SahiFARFactor{} (\SahiBaseFAR{} to
\SahiSlicedFAR), and cost \SahiLatencyFactor$\times$ the latency
(\SI{\SahiBaseLatency}{\milli\second} to
\SI{\SahiSlicedLatency}{\milli\second} per frame). Tiles at full
resolution fire on wave crests and glints the whole-frame pass never
proposed. For a system whose contribution is false-alarm reduction, trading
a thirteenfold rise in false alarms for five points of recall is the wrong
direction; the temporal validator recovers persistent swimmers far more
cheaply. Recorded as a measured negative result.
\end{recorded}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{qual_drowning.png}
\caption{Seven people in open water confirmed simultaneously on a
SeaDronesSee flight sequence. At these altitudes each swimmer is a handful of
pixels, which is why the operating point favours recall and why sliced
inference was tested (and rejected) above. Imagery: SeaDronesSee, CC0.}
\label{fig:qual-drowning}
\end{figure}

\subsubsection{Trapped people and building access}
\label{sec:building}
...
\begin{description}[leftmargin=7.4em,labelwidth=7.0em,style=sameline,font=\normalfont\itshape]
  ...
  \item[Envelope] Downward and oblique UAV views of earthquake-damaged urban
    ...
    cities is unproven and treated as unproven.
\end{description}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{qual_building.png}
\caption{Building-access \emph{detector output only}, shown at confidence
$\ge 0.45$ for legibility. Unlike the other three panels this shows no
confirmed state: the released DRespNeT footage has no temporal continuity, so
the validator cannot run on it. This is the honest limit of
this hazard, drawn rather than described. Imagery: DRespNeT, Cranfield
University, CC~BY~4.0; UAV imagery of the 2023 Türkiye earthquakes.}
\label{fig:qual-building}
\end{figure}

\subsubsection{Road incidents}
\label{sec:traffic}
...
\begin{recorded}{A checkpoint choice recorded, not silently made}
...
reason here so a future reader need not wonder why the two
disagree.
\end{recorded}
