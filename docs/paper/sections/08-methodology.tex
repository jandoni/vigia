...
\texttt{eval/results/}, and every figure in this document is generated from
them (\cref{sec:reproducibility}).
...
Demonstration clips are treated as evidence rather than decoration, and are
...
is rendered directly from the pipeline rather than screen-captured, which
...
\begin{enumerate}
  \item \textbf{The licence permits publication.} The source datasets carry
        ...
        burned onto every frame rather than left to a caption that may be
        cropped away.
...
artefacts. Continuity is now verified rather than assumed: adjacent-frame
...
Flood was solved by changing source rather than by relaxing the rule. The USGS
...
detector's declared envelope, and the fetch script enforces it rather than
restating it.
